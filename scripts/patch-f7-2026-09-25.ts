#!/usr/bin/env tsx
/**
 * scripts/patch-f7-2026-09-25.ts — 金曜ワンセット#7 ⑦(c)(d) の microCMS PATCH（差分限定・件別）
 *
 ...
 * ★POST / PUT / DELETE なし。
 *
 * 実行: set -a && . ./.env.local && set +a && npx tsx scripts/patch-f7-2026-09-25.ts [--dry-run] [--only <id>] [--prefix <id-prefix>] [--skip <id,id,...>]
 */
...
const PLAN = path.join(process.cwd(), 'scripts', 'f7-patch-plan-2026-09-25.json');
const LOG = path.join(process.cwd(), 'scripts', `f7-patch-log-2026-09-25${PREFIX ? `-${PREFIX}` : ''}.json`);
