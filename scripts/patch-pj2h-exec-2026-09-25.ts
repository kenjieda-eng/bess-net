#!/usr/bin/env tsx
/**
 * scripts/patch-pj2h-exec-2026-09-25.ts — Pj2-H 実行便（104行裁定の適用）の microCMS PATCH（差分限定・件別）
 *
 ...
 * ★POST / PUT / DELETE なし。
 *
 * 実行: set -a && . ./.env.local && set +a && npx tsx scripts/patch-pj2h-exec-2026-09-25.ts [--dry-run] [--only <id>] [--prefix <id-prefix>] [--skip <id,id,...>]
 */
...
const PLAN = path.join(process.cwd(), 'scripts', 'pj2h-exec-patch-plan-2026-09-25.json');
const LOG = path.join(process.cwd(), 'scripts', `pj2h-exec-patch-log-2026-09-25${PREFIX ? `-${PREFIX}` : ''}.json`);
...
type Op = {
  ...
  slug: string;
  field: string;
  op: 'set' | 'replace' | 'replaceAll' | 'setRaw' | 'append';
  from: string;
  to: string;
  count?: number;
  why: string;
...
async function main() {
  ...
  for (const o of ops) {
    ...
    // Pj2-H 実行便で追加: op "setRaw" は to を JSON リテラルとして送る（null・数値・配列を型どおりに入れるため）。
    // 数値フィールド（outputMw / capacityMwh）に文字列を入れない・null 化を確実に行うのが目的。
    if (o.op === 'setRaw') {
      const want = JSON.parse(o.to) as unknown;
      const curNorm = norm(raw === undefined ? null : raw);
      if (curNorm === norm(want)) { console.log(`- ${tag}: 適用済み（skip）`); skipped++; log.push({ ...o, result: 'already' }); continue; }
      if (curNorm !== norm(JSON.parse(o.from))) {
        console.log(`✗ ${tag}: 現在値が計画の from と違う（中止）\n    現在: ${curNorm}\n    計画: ${o.from}`);
        failed++; log.push({ ...o, result: 'from-mismatch', before: curNorm }); continue;
      }
      console.log(`■ ${tag}\n    前: ${curNorm}\n    後: ${o.to}\n    理由: ${o.why}`);
      if (DRY) { ok++; log.push({ ...o, result: 'dry-run', before: curNorm }); continue; }
      try {
        await api('PATCH', `${ep(o.endpoint)}/${b.id}`, { [o.field]: want });
      } catch (e) {
        console.log(`✗ ${tag}: PATCH 失敗（変更なし） ${String(e).slice(0, 200)}`); failed++;
        log.push({ ...o, result: 'patch-error', before: curNorm, note: String(e).slice(0, 300) }); continue;
      }
      await sleep(900);
      const a2 = await getRec(o.endpoint, o.slug);
      let other2 = 0;
      for (const k of new Set([...Object.keys(b), ...Object.keys(a2 ?? {})])) {
        if (SYS.has(k) || k === o.field) continue;
        if (norm(a2?.[k]) !== norm(b[k])) { other2++; console.log(`    ✗ 対象外フィールドが変化: ${k}`); }
      }
      const hit2 = norm(a2?.[o.field] === undefined ? null : a2?.[o.field]) === norm(want);
      console.log(`    #106: ${hit2 ? `✓ 対象 ${o.field} 反映` : `★NG 反映されていない`}・他フィールド変化 ${other2}`);
      if (hit2 && other2 === 0) ok++; else failed++;
      log.push({ ...o, result: hit2 && other2 === 0 ? 'ok' : 'verify-ng', before: curNorm, after: norm(a2?.[o.field]), otherFieldChanges: other2 });
      fs.writeFileSync(LOG, JSON.stringify(log, null, 1));
      await sleep(300);
      continue;
    }
    // Pj2-H 実行便で追加: op "append" は本文末尾に段落を足す（裁定書 §5「値を null にする行は body に帰属つきで残す」）。
    // ★#122: 冪等判定は「送信 HTML の全文一致」ではなく marker（o.from に入れた素の文言）の有無で行う。
    if (o.op === 'append') {
      const marker = o.from;
      if (!marker) { console.log(`✗ ${tag}: append には marker（from）が必要`); failed++; log.push({ ...o, result: 'no-marker' }); continue; }
      if (cur.includes(marker)) { console.log(`- ${tag}: 適用済み（marker あり・skip）`); skipped++; log.push({ ...o, result: 'already' }); continue; }
      const appended = cur + o.to;
      console.log(`■ ${tag}\n    末尾に追記（${cur.length}→${appended.length}字）: ${o.to.slice(0, 160)}\n    理由: ${o.why}`);
      if (DRY) { ok++; log.push({ ...o, result: 'dry-run', before: `${cur.length}字` }); continue; }
      try {
        await api('PATCH', `${ep(o.endpoint)}/${b.id}`, { [o.field]: appended });
      } catch (e) {
        console.log(`✗ ${tag}: PATCH 失敗（変更なし） ${String(e).slice(0, 200)}`); failed++;
        log.push({ ...o, result: 'patch-error', note: String(e).slice(0, 300) }); continue;
      }
      await sleep(900);
      const a3 = await getRec(o.endpoint, o.slug);
      let other3 = 0;
      for (const k of new Set([...Object.keys(b), ...Object.keys(a3 ?? {})])) {
        if (SYS.has(k) || k === o.field) continue;
        if (norm(a3?.[k]) !== norm(b[k])) { other3++; console.log(`    ✗ 対象外フィールドが変化: ${k}`); }
      }
      const raw3 = String(a3?.[o.field] ?? '');
      const cur3 = RICH.has(`${o.endpoint}.${o.field}`) ? decode(raw3) : raw3;
      const hit3 = cur3.includes(marker);
      console.log(`    #106/#122: ${hit3 ? '✓ marker あり（richEditor の正規化は許容）' : '★NG marker 不在'}・他フィールド変化 ${other3}`);
      if (hit3 && other3 === 0) ok++; else failed++;
      log.push({ ...o, result: hit3 && other3 === 0 ? 'ok' : 'verify-ng', otherFieldChanges: other3 });
      fs.writeFileSync(LOG, JSON.stringify(log, null, 1));
      await sleep(300);
      continue;
    }
