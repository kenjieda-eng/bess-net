#!/usr/bin/env tsx
/**
 * scripts/patch-pj2h0-2026-09-24.ts — Pj2-H-0 構造便の microCMS PATCH（差分限定・件別）
 *
 ...
 * ★POST / PUT / DELETE なし。
 *
 * 実行: set -a && . ./.env.local && set +a && npx tsx scripts/patch-pj2h0-2026-09-24.ts [--dry-run] [--only <id>] [--prefix <id-prefix>] [--skip <id,id,...>]
 */
...
const PLAN = path.join(process.cwd(), 'scripts', 'pj2h0-patch-plan-2026-09-24.json');
const LOG = path.join(process.cwd(), 'scripts', `pj2h0-patch-log-2026-09-24${PREFIX ? `-${PREFIX}` : ''}.json`);
