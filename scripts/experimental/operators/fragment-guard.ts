 ...
 *   ★2026-08-24 に operators マスタの社名を「株式会社ミライト」→「株式会社ミライト・ワン」へ改名した
 *     （旧称は 2022-07-01 の3社統合で消滅）。これにより (a)「既存社名＋ノイズ語」型の偽陽性は
 *     根本解消しており、本 allowlist の目的は下記システムズとの区別のみになった。
 * ・株式会社ミライト・ワン・システムズ … 2026年8月時点で別法人（東京都港区・未上場・2022年7月設立・
 *   ソフトウェア）。**2026年10月1日（予定）でミライト・ワンに吸収合併され解散**する
 *   （決議 2026-05-12・出典 https://www.nihon-ma.co.jp/news/20260512_1417-22/ ）。
 *   ★時点明示で書く（#123 / L-EIC-027）: 「別法人（現存）」と書くと 2026-10-01 以降 stale になる。
 *   合併後は「ミライト・ワンへの承継」となるため、10/1 以降にマスタで扱う場合は
 *   本エントリの要否を再判断すること（それまでは双方向の誤マージ防止のため両方を登録しておく）。
 */
