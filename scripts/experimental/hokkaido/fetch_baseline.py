# -*- coding: utf-8 -*-
"""東北エリアの既存レコードを microCMS から GET してローカル保存（読取専用・書込ゼロ）。
...
while True:
    url = (f"https://{DOMAIN}.microcms.io/api/v1/substations?limit=100&offset={offset}"
           f"&filters=area%5Bcontains%5D%E5%8C%97%E6%B5%B7%E9%81%93")
    ...
    offset += 100
    time.sleep(0.4)
json.dump(out, open("scripts/experimental/hokkaido/baseline_live.json", "w", encoding="utf-8"),
