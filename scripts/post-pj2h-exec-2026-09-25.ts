#!/usr/bin/env tsx
/**
 * scripts/post-pj2h-exec-2026-09-25.ts — Pj2-H 実行便 A-1/A-4 の projects POST（15 件）
 *
 * 計画: scripts/pj2h-exec-post-plan-2026-09-25.json（journal の検証済み出力から機械生成。転記ミス防止）
 ...
 *
 * 実行: set -a && . ./.env.local && set +a && npx tsx scripts/post-pj2h-exec-2026-09-25.ts [--dry-run]
 */
...
const PLAN = path.join(process.cwd(), 'scripts', 'pj2h-exec-post-plan-2026-09-25.json');
const LOG = path.join(process.cwd(), 'scripts', 'pj2h-exec-post-log-2026-09-25.json');
