...
【受け入れ基準の恒久追加（容量市場の年度表記・2026-09-21 Nv-0c）】
  ✅ 容量市場の年度は必ず「対象実需給年度」か「実施年度（応札年度）」を明記する。
     OCCTO 自身が両方を使うため「2025年度」単独は曖昧。
     カタログ `capacity-main-auction-price-*` の date は対象実需給年度。
     （メインオークションは対象実需給年度の 4 年前に実施＝実施2025 は対象実需給2029）
     実証: Nv-0b で容量市場文脈の曖昧表記を src 27・microCMS 27 の計 54 箇所検出。

【受け入れ基準の恒久追加（資料名・2026-09-21 Ck-1）】
  ✅ 資料名は書く前に一次の `<title>` を実機で取り、台帳に確認日つきで登録する。指示文にあった名前をそのまま実装しない。
     台帳: src/data/source-documents.json（資料名 → 発行元・URL・確認時の title 逐語・確認日）。
...
#124 ★★★★ 一覧の並びが「参照元 API の登録順」に暗黙依存していると、データを別 API へ移した
           瞬間に表面化する。ソート指定には必ず決定的なタイブレークを置く
           → microCMS の `orders: '-eventDate'` は**同一日の二次ソートを定義しない**。タイは
             内部順（実質は登録順）に落ちるため、一覧の並びが「どの順で登録したか」に依存する。
             データが同じ API に留まっている限り表に出ないので、**移行するまで気づけない**。
           実証: 2026-08-31 の API 統合（industry-events → policy-events）。41件を greedy set
             cover 順（選択肢検証を最短で終える順）で投入したところ、旧APIの登録順と違うため
             /events の同着日 11箇所が入れ替わった。集合・件数・日付表示はすべて不変で、
             #106 照合も 41/41 一致＝**データは正しく、並びの決まり方だけが暗黙だった**。
           → 旧APIの登録順を再現する対処は不可（削除予定のAPIの登録順に依存し続け、次に1件
             足した瞬間にまた崩れる）。決定的なタイブレークを置くのが唯一の恒久策。
             採用: eventDate 降順 → title（localeCompare('ja')）→ slug。
             Gr11 の /grid/search（空容量降順 → 名称 → slug）と同じ「意味のあるキー → 名称 →
             slug」型に揃え、サイト全体で並びの決め方を統一する。
           → 実装は共有ヘルパ1箇所（compareEventsForDisplay）に置き、同じ API を読む複数ビューが
             同じ規則になるようにする（#119 と対）。呼び出し側で並べ直さない。
           【受け入れ基準（恒久）】
             ✅ 一覧のソート指定に第二・第三キーがあるか（無ければ内部順に依存している）
             ✅ 並びを変える変更では「同着キー内の入替のみ」であることを実測で確認する。
...
- 2026-09-21（CC・Ck-1 点検残件）: 受け入れ基準に「資料名は書く前に一次の title を実機で取り台帳に登録」（Lc-3 の文言）を追加。台帳 src/data/source-documents.json・検査 npm run verify:source-names を新設。蓄電池の性能前提（往復効率・寿命・DoD）を src/lib/storage-assumptions.ts に一本化（効率 85%・寿命 15 年は NREL ATB 2024 年版本文、DoD は前提値）
- 2026-09-21（CC・Nv-0c 容量市場の既定値）: 受け入れ基準に「容量市場の年度は対象実需給年度か実施年度を明記」を追加（Nv-0b で CC が提示した文言をユウ裁定で採用）。IRR シミュレータの容量市場既定値をカタログ（national 中央値・最大・最小）へ、需給調整は既定値を置かない形へ。旧 8,000・1,500 の出所は 2026-05-14 の議論メモで、いずれも一次に対応が無かった
