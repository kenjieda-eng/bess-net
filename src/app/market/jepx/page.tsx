/**
 * /market/jepx — JEPX ハブ (依頼AN、業界唯一性 +1)
 *
 * 設計 (CLAUDE.md §0 鉄則完全準拠):
 *   - 鉄則 #2: SSR 外部 API 0 (静的 jepx-history データ)
 *   - 鉄則 #3: 単一 URL
 *   - 鉄則 #4: ピーク負荷 0 req/分
 */
...
export const metadata: Metadata = {
  title: 'JEPX ハブ (スポット価格 × 30日ヒートマップ × アービトラージ計算)',
  description: `JEPX スポット市場の業界唯一ハブ。9エリア × 過去 30日 × 30分単位 ${DAILY_DATA.length * 48} データポイント、ヒートマップ可視化、月次推移 ${MONTHLY_DATA.length} 件、蓄電池アービトラージ計算機。無料公開・登録不要。`,
  alternates: { canonical: '/market/jepx' },
  openGraph: {
    title: 'JEPX ハブ (スポット価格 × ヒートマップ × 蓄電池アービトラージ)',
    description: '9エリア × 30日 × 30分 + 月次12ヶ月 + アービトラージ計算機',
    type: 'website',
    images: ['/og-image.png'],
  },
};

export default function JEPXHubPage() {
  // JSON-LD Dataset
  const datasetJsonLd = {
    '@context': 'https://schema.org',
    '@type': 'Dataset',
    name: 'JEPX スポット価格 ハブ データセット (モック版)',
    description: `9エリア × 過去 30日 × 30分単位 (合計 ${DAILY_DATA.length * 48} データポイント) + 月次集計 ${MONTHLY_DATA.length} 件`,
    keywords: ['JEPX', 'スポット市場', '電力市場', 'アービトラージ', '蓄電池'],
      ...
      <main className="section">
        <div className="section-inner">
          ...
          <p className="section-desc" style={{ marginBottom: 16 }}>
            JEPX スポット市場を<strong>9エリア × 過去 30日 × 30分単位</strong> で可視化。
            <strong>ヒートマップ</strong>で 24h × 30日の価格分布を一望、
            <strong>月次12ヶ月</strong>で季節性を把握、
            <strong>アービトラージ計算機</strong>で蓄電池 (容量/効率/サイクル数) の日次粗利益を試算。
            業界唯一のスポット価格ハブ、無料公開・登録不要。
          </p>
          <p className="page-meta" style={{ fontSize: 13, color: 'var(--color-muted)', marginTop: 0, marginBottom: 24 }}>
            ※ 本データは JEPX 公表値ベースの<strong>モックデータ</strong>です (構造は本物に準拠、{AREAS.length}エリア × 30日 × 48 slot)。
            最新の実勢値は{' '}
