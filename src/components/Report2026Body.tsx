  ...
  return (
    <div className="report-body" style={{ marginTop: 40 }}>
      ...
      <section>
        ...
        <p style={P}>
          2025-2026は、その制度が実際の投資と建設に結びつく<strong>本格導入の入口</strong>である。当サイトのプロジェクトDBには{n(projectCount)}件が登録され、稼働（{n(st('稼働中'))}件）と計画・建設（{n(pipeline)}件）がほぼ同数で並ぶ。担い手も、上場企業・電力会社・商社・通信・独立系・海外資本・地域企業へと多様化した。
        </p>
        <p style={P}>
          本レポートは、当サイトが業界唯一機能（プロジェクトDB・事業者ナビ・系統空き容量・補助金カレンダー・用語集・火災事例DB・海外市場ハブ等）で蓄積したデータを編集統合し、市場・政策・プレイヤー・系統・補助金・安全・海外・展望を1冊に束ねたものである。<strong>分析</strong>：本質は「変動する安い再エネを、必要なときに使える電力へ変える」こと。この一点に、蓄電所事業のすべての収益機会と社会的意義が集約される。
        </p>
        <p style={LINKS}>
      ...
      <section>
        ...
        <p style={P}>
          立地選定の出発点は「系統に繋げるか」である。当サイトは9送配電エリア<strong>{n(substationCount)}変電所</strong>の系統空き容量・運用容量・予想潮流・N-1電制適用可否・ノンファーム接続可否を収録する、業界唯一の統合データベースを提供している（関東＝東京電力パワーグリッドは系統情報公開停止の経緯があり、代替アクセスの解説を別途用意）。
        </p>
        <p style={P}>
